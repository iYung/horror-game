...
-- Compass (value=0) always spawns exactly once, guaranteed > 10 cells from spawn.
--
...
function ItemSpawner.spawn(map, budget)
    ...
    if budget >= 1 then
        ...
        local cells = pick_random(walkable, fl_count, reserved)
    ...
    for _, cell in ipairs(walkable) do
        if manhattan(cell.col, cell.row, sc, sr) > 10 then
            local k = cell.col * 1000 + cell.row
            if not reserved[k] then
                table.insert(compass_pool, cell)
            end
        end
    end

    if #compass_pool == 0 then
        for _, cell in ipairs(walkable) do
            local k = cell.col * 1000 + cell.row
            if not reserved[k] then
                table.insert(compass_pool, cell)
    ...
    if #compass_pool > 0 then
        local idx  = math.random(1, #compass_pool)
        local cell = compass_pool[idx]
        local wx, wy = cell_centre(cell.col, cell.row)
